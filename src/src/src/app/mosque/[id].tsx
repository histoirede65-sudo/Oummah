...
export default function MosqueDetailScreen() {
  ...
  const openPrayerTimesProposal = () => {
    setPrayerForm({
      ...
      maghrib: prayerTimes?.maghrib ?? '', isha: prayerTimes?.isha ?? '', jumuah: prayerTimes?.jumuah ?? '', note: '',
    });
  ...
  const submitPrayerTimesProposal = async () => {
    ...
    try {
      ...
      Alert.alert('Envoi impossible', error instanceof Error && error.message === 'AUTH_REQUIRED' ? 'Connectez-vous pour proposer des horaires.' : 'Vérifiez que les horaires sont au format HH:MM.');
      ...
      <Modal visible={prayerModalVisible} transparent animationType="slide" onRequestClose={() => setPrayerModalVisible(false)}>
        <View style={styles.modalBackdrop}>
          <View style={styles.prayerModal}>
            ...
            <Text style={styles.prayerModalText}>Ils seront publiés uniquement après validation.</Text>
            <View style={styles.prayerInputsGrid}>
              {(['fajr','dhuhr','asr','maghrib','isha','jumuah'] as const).map((key) => (
                <View key={key} style={styles.prayerInputBlock}>
                  ...
                  <TextInput value={prayerForm[key]} onChangeText={(value) => setPrayerForm((current) => ({...current,[key]:value}))} placeholder="HH:MM" placeholderTextColor="#837789" keyboardType="numbers-and-punctuation" style={styles.prayerInput} />
                </View>
              ))}
            </View>
...
const styles = StyleSheet.create({
  ...
  modalBackdrop: { flex: 1, justifyContent: 'flex-end', backgroundColor: 'rgba(7,5,11,0.72)' },
