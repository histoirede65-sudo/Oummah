...
export default function HomeGoalsSection() {
  ...
  const [hasConfiguredGoals, setHasConfiguredGoals] = useState(false);

  useFocusEffect(
    useCallback(() => {
      let active = true;
      Promise.all([getValidSession(), goalsModel.refresh().then(() => goalsModel).catch(() => null)])
        .then(async ([session]) => {
          if (!active) return;
          setIsAuthenticated(Boolean(session));
          if (!session) {
            setHasConfiguredGoals(false);
            return;
          }
          const settings = await goalsModel.readSettings();
          if (active) setHasConfiguredGoals(Boolean(settings.onboardingComplete));
        })
        .catch(() => {
          if (active) {
    ...
    <View style={styles.row}>
      ...
      <Pressable
        ...
        style={({ pressed }) => [
          styles.card,
          styles.goalsCard,
          (!isAuthenticated || !hasConfiguredGoals) && styles.signupCard,
          pressed && styles.pressed,
        ]}
      >
        {!isAuthenticated || !hasConfiguredGoals ? (
          <View style={styles.signupContent}>
            <View style={styles.signupIcon}>
              <Ionicons name="person-add-outline" size={21} color="#16111B" />
            </View>
            <Text style={styles.signupTitle}>{isAuthenticated ? "Créez votre premier objectif" : "Créez vos objectifs"}</Text>
            <Text style={styles.signupText}>
              {isAuthenticated
                ? "Choisissez un rythme et des objectifs adaptés pour commencer votre progression."
                : "Inscrivez-vous gratuitement pour créer vos objectifs et suivre votre progression."}
            </Text>
            <Text style={styles.signupLink}>{isAuthenticated ? "Créer mes objectifs →" : "Créer mon profil →"}</Text>
          </View>
        ) : (
        <>
