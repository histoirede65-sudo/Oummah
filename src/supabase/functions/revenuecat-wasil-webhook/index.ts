...
Deno.serve(async (request) => {
  ...
  const isProductionGooglePlay =
    environment === "PRODUCTION" &&
    (store === "PLAY_STORE" || store === "GOOGLE_PLAY");
  const isSandboxTest = environment === "SANDBOX" && store === "TEST_STORE";
  const purchaseEnvironment = isProductionGooglePlay ? "production" : "test";
  const purchasePlatform = isProductionGooglePlay ? "google_play" : "revenuecat_test";
  ...
  if (
    ...
    !productId ||
    !transactionId ||
    (!isProductionGooglePlay && !isSandboxTest) ||
  ...
  const { data, error } = await supabase.rpc("grant_wasil_purchase_credits", {
    ...
    p_environment: purchaseEnvironment,
    p_platform: purchasePlatform,
