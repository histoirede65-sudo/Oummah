...
import { getHadithRepositoryDebug } from "./engine/repositories/HadithRepository.ts";
...
function analyzeWasilQuery(question: string, mode: "standard" | "deep"): WasilQueryProfile {
  ...
  const depth: WasilQueryProfile["depth"] = asksForShort
    ...
    : asksForDetail
      ? "detailed"
      : "standard";
  const maxOutputTokens = depth === "short" ? 900 : depth === "detailed" ? 3600 : 2200;
...
function parseQuranReference(value: string): QuranReference | null {
  const match = value.match(/(?:coran\s*)?(\d{1,3})\s*:\s*(\d{1,3})(?:\s*[-–—]\s*(\d{1,3}))?/iu);
...
function quranReferenceKey(reference: QuranReference): string {
...
function explicitlyRequestsQuranAndSunnah(question: string): boolean {
  ...
  const requestsSunnah = /\b(?:sunna|sunnah|hadiths?)\b/.test(normalized);
  return requestsQuran && requestsSunnah;
}

const PROMISE_QURAN_REFERENCE_KEYS = new Set([
  "5:1:1",
  "16:91:91",
  "17:34:34",
  "23:8:8",
...
function isPromiseOrCovenantQuestion(question: string): boolean {
  return /\b(?:promesse|promesses|promet|promis|engagement|engagements|pacte|pactes|contrat|contrats|trahit|trahison)\b/.test(
    normalizeQuestion(question),
  );
}

function prioritizeDirectPromiseEvidence(input: {
  question: string;
  parsedSourceIds: string[];
  parsedQuranReferences: QuranReference[];
  requestSources: Record<string, TrustedSource>;
  productionHadithMetadata: Map<string, HadithReference>;
}): void {
  if (!isPromiseOrCovenantQuestion(input.question)) return;

  const directQuranIds = Object.entries(input.requestSources)
    .filter(([, source]) => {
      const reference = parseQuranReference(source.reference);
      return reference ? PROMISE_QURAN_REFERENCE_KEYS.has(quranReferenceKey(reference)) : false;
    })
    .map(([sourceId]) => sourceId);

  // When direct normative verses are available, they replace indirect
  // narrative passages such as At-Tawba 9:117-119 in the displayed cards.
  // Non-Quran sources are preserved.
  if (directQuranIds.length > 0) {
    const nonQuranIds = input.parsedSourceIds.filter((sourceId) => {
      const source = input.requestSources[sourceId];
      return !source || !parseQuranReference(source.reference);
    });
    ...
    input.parsedQuranReferences.splice(
      0,
      input.parsedQuranReferences.length,
      ...directQuranIds
        .map((sourceId) => parseQuranReference(input.requestSources[sourceId].reference))
        .filter((reference): reference is QuranReference => Boolean(reference)),
    );
  }

  // Prefer the deterministic HadeethEnc entry about broken promises whenever
  // it is available. A long historical narration can remain only when the
  // direct hadith was not retrieved.
  const directHadithIds = [...input.productionHadithMetadata.entries()]
    .filter(([, reference]) => reference.id === "66537")
    .map(([sourceId]) => sourceId);
  if (directHadithIds.length > 0) {
    const withoutOtherHadiths = input.parsedSourceIds.filter(
      (sourceId) => !input.productionHadithMetadata.has(sourceId),
    );
    input.parsedSourceIds.splice(
      0,
      input.parsedSourceIds.length,
...
const DIRECT_ANGER_HADITH_IDS = new Set(["4709", "5351"]);

function isAngerQuestion(question: string): boolean {
  return /\b(?:colere|enerve|enervement|irrite|irritation|emporte|emportement|maitrise de soi)\b/.test(
    normalizeQuestion(question),
  );
}

function prioritizeDirectAngerEvidence(input: {
  question: string;
  parsedSourceIds: string[];
  productionHadithMetadata: Map<string, HadithReference>;
}): void {
  if (!isAngerQuestion(input.question)) return;

  const directHadithIds = [...input.productionHadithMetadata.entries()]
    .filter(([, reference]) => reference.id && DIRECT_ANGER_HADITH_IDS.has(reference.id))
    .map(([sourceId]) => sourceId);
  if (directHadithIds.length === 0) return;

  const withoutIndirectHadiths = input.parsedSourceIds.filter(
    (sourceId) => !input.productionHadithMetadata.has(sourceId),
  );
  input.parsedSourceIds.splice(
    0,
    input.parsedSourceIds.length,
    ...withoutIndirectHadiths,
    ...directHadithIds,
  );
}

function ensureComplementaryQuranSources(input: {
  question: string;
  parsedSourceIds: string[];
  parsedQuranReferences: QuranReference[];
  requestSources: Record<string, TrustedSource>;
  brainPlan: WasilV4ShadowResult["brainPlan"];
  documentaryQuranSourceIds: string[];
  documentaryHadithSourceIds: string[];
}): void {
  const plannedSkills = new Set(
    input.brainPlan?.executionSteps.map((step) => step.skill) ?? [],
  );
  const requestRequiresBoth = explicitlyRequestsQuranAndSunnah(input.question);
  const planRequiresBoth = plannedSkills.has("quran") && plannedSkills.has("hadith");
  if (!planRequiresBoth && !requestRequiresBoth) return;

  const existingReferenceKeys = new Set(
    input.parsedQuranReferences.map(quranReferenceKey),
  );
  const existingSourceIds = new Set(input.parsedSourceIds);
  for (const sourceId of input.documentaryHadithSourceIds) {
    if (input.requestSources[sourceId] && !existingSourceIds.has(sourceId)) {
      input.parsedSourceIds.push(sourceId);
      existingSourceIds.add(sourceId);
    }
  }

  // Complement only with Quran passages returned by the documentary Quran
  // engine. Do not sweep every local Quran source present in requestSources:
  // those may merely be broad candidates and were the cause of recurring,
  // unrelated cards such as 5:6, 4:103 and 33:40.
  for (const sourceId of input.documentaryQuranSourceIds) {
    const source = input.requestSources[sourceId];
    if (!source) continue;
...
function deduplicateQuranReferences(
  references: QuranReference[],
): QuranReference[] {
  const byKey = new Map<string, QuranReference>();
  ...
  for (const rawReference of references) {
    ...
    const normalized: QuranReference = {
      surah,
      verseStart,
      // A one-verse range and a null end represent the same passage.
      verseEnd: verseEnd === verseStart ? null : verseEnd,
    };
    const key = quranReferenceKey(normalized);
...
function deduplicateSelectedQuranSourceIds(
  sourceIds: string[],
  requestSources: Record<string, TrustedSource>,
): string[] {
  const seenSourceIds = new Set<string>();
  const seenQuranReferences = new Set<string>();
  const result: string[] = [];
  ...
  for (const sourceId of sourceIds) {
    ...
    const parsedReference = source
      ? parseQuranReference(source.reference)
      : null;
    if (parsedReference) {
      const key = quranReferenceKey(parsedReference);
      if (seenQuranReferences.has(key)) continue;
      seenQuranReferences.add(key);
    }
...
function pickVerifiedConsultedReferences(
  ...
  const exact = requested
    ...
    .filter((source): source is WebReference => Boolean(source) && isAllowed(source));
...
Deno.serve(async (request) => {
  ...
  if (
    ...
    v4Analysis = await runWasilV4ShadowPipeline(effectiveQuestion, requestId);
  } else {
    // Pure shadow mode remains fire-and-forget and cannot affect production.
    void runWasilV4ShadowPipeline(effectiveQuestion, requestId);
  }
  ...
  try {
    ...
    const executionPlan: WasilProductionExecutionPlan | null =
      featureFlags.v4ExecutionPlan
        ...
        : null;
    const directQuranTopic = await retrieveQuranKnowledgeSafely(effectiveQuestion);
    const queryExpansion = directQuranTopic
      ? null
      : await expandIslamicQuery(effectiveQuestion);
    const quranTopic = directQuranTopic ?? await retrieveQuranKnowledgeSafely(
      effectiveQuestion,
      queryExpansion,
    );
    ...
    const documentaryQuranSourceIds = [
      ...Object.keys(quranTopic?.sources ?? {}),
      ...(v4Analysis?.quranRecord?.passages.map((passage) => passage.sourceId) ??
        []),
    ].filter((sourceId, index, all) => all.indexOf(sourceId) === index);
    const documentaryHadithSourceIds = [
      ...productionHadith.metadata.keys(),
    ];
    ...
    const localHadithSourceCount = requestSourceEntriesBeforePrompt.filter(
      ([sourceId]) =>
        sourceId.startsWith("v4-hadith:") || sourceId.startsWith("hadith:"),
    ).length;
    ...
    const useWebSearch = stableUseWebSearch || Boolean(executionPlan?.shouldUseWeb);
    ...
    const brainGuidance =
      featureFlags.v4ProductionBrainGuidance || featureFlags.v4ExecutionPlan
        ...
        : "";
    const promiseEvidenceGuidance = isPromiseOrCovenantQuestion(effectiveQuestion)
      ? " Pour les promesses, pactes et engagements, privilégie les preuves normatives directes (Coran 5:1, 16:91, 17:34, 23:8 lorsqu’elles sont fournies) et le hadith direct sur le fait de manquer à sa promesse. Un récit historique long, notamment celui de Ka‘b ibn Mâlik, ne doit être utilisé qu’en illustration secondaire et jamais remplacer ces preuves directes."
      : "";
    const productionInstructions = `${stableInstructions}${brainGuidance}\n\nRÈGLE DOCUMENTAIRE PR16: avant de rédiger une réponse religieuse substantielle, examine les preuves coraniques et les hadiths fournis lorsque le plan les prévoit. Utilise les deux lorsqu’ils sont réellement complémentaires, sans ajouter de citation décorative. Les cartes Hadith sont générées uniquement depuis les SOURCE_ID commençant par v4-hadith:. N’invente jamais de collection, de numéro ni de requête de navigation. Sélectionne ces SOURCE_ID seulement si le hadith est réellement utilisé dans le corps.${promiseEvidenceGuidance}`;
    ...
    ensureComplementaryQuranSources({
      question: effectiveQuestion,
    ...
    prioritizeDirectPromiseEvidence({
      ...
      parsedQuranReferences: parsed.quran_references,
      requestSources,
      productionHadithMetadata: productionHadith.metadata,
    });
    ...
    const finalAnswerBody = requestedQuranAndSunnah && hadithReferences.length === 0
