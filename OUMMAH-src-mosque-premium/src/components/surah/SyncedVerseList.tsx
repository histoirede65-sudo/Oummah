...
import { ARABIC_READING_COLOR, ARABIC_READING_FONT_FAMILY, ARABIC_READING_FONT_WEIGHT, getArabicReadingMetrics } from '../../features/quran/ArabicReadingPresentation';
...
import { activeVerseAt, getActiveWordTimestamp, getSyncPositionMs, getWordSyncState, normalizeVerseTimestamps, normalizeWordTimestamps } from '../../features/quran/QuranWordSync';
import type { AudioSourceMode, VerseTimestamp, WordTimestamp } from '../../features/quran/QuranWordSync';
...
function verseKey(verse: QuranFoundationVerse) {
  return verse.verseKey ?? (verse as QuranFoundationVerse & { verse_key?: string }).verse_key;
}
...
function verseWords(verse: QuranFoundationVerse) {
  const foundationWords = (verse.words ?? []).flatMap((word): SyncedWord[] => {
    ...
    if (charType !== 'word' || !Number.isFinite(position) || position <= 0 || !text.trim()) return [];
...
function verseTranslation(verse: QuranFoundationVerse) {
  return verse.translations?.[0]?.text || verse.translation || '';
}
...
function restoreOpenFinalSegments(
  ...
  raw.forEach((item) => {
    ...
    const verseId = verseNumberFromKey(String(record.verseKey ?? record.verse_key ?? ''));
    const verseEndMs = Number(record.timestampTo ?? record.timestamp_to);
    const segments = record.segments;
    if (!verseId || !Number.isFinite(verseEndMs) || !Array.isArray(segments)) return;
    const finalSegment = [...segments].reverse().find((segment) => Array.isArray(segment) && segment.length >= 3);
    if (!Array.isArray(finalSegment) || finalSegment[2] !== null && finalSegment[2] !== undefined) return;
    const wordPosition = Number(finalSegment[0]);
    const startMs = Number(finalSegment[1]);
    if (!Number.isFinite(wordPosition) || !Number.isFinite(startMs) || verseEndMs <= startMs) return;
    if (restored.some((word) => word.verseId === verseId && word.wordPosition === wordPosition)) return;
...
export default function SyncedVerseList({
  ...
  const [rawTiming, setRawTiming] = useState<{ key: string; timestamps: unknown } | null>(null);
  ...
  const rawTimestamps = rawTiming?.key === timingKey ? rawTiming.timestamps : null;
  ...
  useEffect(() => {
    ...
    void quranFoundationRepository.getVerses(surahId)
      .then((nextVerses) => {
      ...
      .catch(() => undefined)
      .finally(() => {
        if (active && syncSessionIdRef.current === syncSessionId) setLoading(false);
      });
  ...
  useEffect(() => {
    ...
    if (reciterId) {
      void quranFoundationRepository.getRecitation(reciterId, surahId)
        .then((recitation) => {
          if (!active || syncSessionIdRef.current !== syncSessionId || timingRequestIdentityRef.current !== requestIdentity) return;
          if (isRemoteAudioUrl(audioUrl) && recitation?.audioUrl && recitation.audioUrl !== audioUrl) return;
          setRawTiming({ key: requestIdentity, timestamps: recitation?.timestamps ?? null });
        })
        .catch(() => undefined);
    }
  ...
  const syncedVerses = useMemo<readonly SyncedVerse[]>(() => {
    const timestampByVerse = new Map(timestamps.map((item) => [item.verseId, item]));
    return verses.map((verse) => {
      ...
      return {
        id,
        arabic: words.length > 0 ? words.map((word) => word.text).join(' ') : clipWords(verseArabic(verse), 9),
        words,
  ...
  const activeVerse = useMemo(() => (
    syncedVerses.find((verse) => verse.id === listenerWordState.activeVerseId)
      ?? syncedVerses[0]
      ?? null
  ...
  const visibleVerses = useMemo(() => {
    ...
    const foundIndex = syncedVerses.findIndex((verse) => verse.id === activeVerse?.id);
    const activeIndex = foundIndex >= 0 ? foundIndex : 0;
    const startIndex = activeIndex === syncedVerses.length - 1
      ? Math.max(0, activeIndex - 1)
      : activeIndex;
    return syncedVerses.slice(startIndex, startIndex + 2);
  }, [activeVerse?.id, syncedVerses]);
  verseKeyRef.current = listenerWordState.activeVerseId ?? syncedVerses[0]?.id ?? null;
  ...
  const handlePositionUpdate = useCallback((positionMs: number) => {
    ...
    const currentVerseTimestamp = verseTimelineRef.current.find((timestamp) => (
      positionMs >= timestamp.startSeconds * 1000 && positionMs < timestamp.endSeconds * 1000
    ));
    const verseTimestampFromMs = Math.round((currentVerseTimestamp?.startSeconds ?? 0) * 1000);
    const syncPositionMs = getSyncPositionMs(positionMs, verseTimestampFromMs, mode);
    const timedWord = getActiveWordTimestamp(syncPositionMs, timelineRef.current);
    const activeVerseId = timedWord?.verseId
    ...
    const nextState = getWordSyncState({
      ...
      verseTimeline: timelineRef.current.filter((word) => word.verseId === activeVerseId),
    });
    setListenerWordState((previous) => {
      const sameCompleted = previous.completedWordPositions.length === nextState.completedWordPositions.length
        && previous.completedWordPositions.every((position, index) => position === nextState.completedWordPositions[index]);
  ...
  useEffect(() => {
    ...
    return unsubscribe;
  }, [getCurrentPositionMs, handlePositionUpdate, subscribeToPosition, syncSessionId]);

  const activeWordState = listenerWordState;
  const lastReadWordPosition = activeWordState.completedWordPositions.at(-1) ?? null;

  const visibleFontsLoaded = visibleVerses.every((verse) => loadedFonts.has(verse.fontFamily) || Font.isLoaded(verse.fontFamily));
  useEffect(() => {
    if (!reciterId || !rawTimestamps || wordTimestamps.length === 0 || syncedVerses.length === 0 || !visibleFontsLoaded) return;
    handlePositionUpdate(getCurrentPositionMs());
    onTimelineReady?.(`${reciterId}:${surahId}`);
  }, [getCurrentPositionMs, handlePositionUpdate, onTimelineReady, rawTimestamps, reciterId, surahId, syncedVerses.length, visibleFontsLoaded, wordTimestamps.length]);
  useEffect(() => {
    let active = true;
    const missingFonts = visibleVerses.filter((verse) => !loadedFonts.has(verse.fontFamily) && !Font.isLoaded(verse.fontFamily));
    ...
    const fontMap = missingFonts.reduce<Record<string, number>>((accumulator, verse) => {
  ...
  if (!visibleFontsLoaded || syncedVerses.length === 0) {
    return (
      <View style={[styles.container, overlay && styles.overlayContainer, compact && styles.containerCompact, styles.emptyContainer]}>
        <Text style={styles.emptyTitle}>{!visibleFontsLoaded || loading ? 'Chargement des versets…' : 'Versets indisponibles'}</Text>
  ...
  return (
    <View style={[styles.container, overlay && styles.overlayContainer, compact && styles.containerCompact]}>
      ...
      <View style={styles.content}>
        {visibleVerses.map((verse) => (
          <SyncedVerseRow
            ...
            mode={mode}
            activeWordPosition={verse.id === activeVerse?.id ? activeWordState.activeWordPosition : null}
            lastReadWordPosition={verse.id === activeVerse?.id ? lastReadWordPosition : null}
            screenWidth={width}
  ...
  return (
    <View style={styles.modeTabs}>
      {tabs.map((tab) => (
        ...
        >
          <Text style={[styles.modeTabText, mode === tab.mode && styles.modeTabTextActive]}>
            {tab.label}
          </Text>
        </Pressable>
      ))}
      ...
      {words.map((word, index) => (
        <Fragment key={`${word.position}-${word.text}`}>
          <QuranWordHighlight
            ...
            isRead={lastReadWordPosition !== null && word.position <= lastReadWordPosition}
            text={word.text}
          />
...
const SyncedVerseRow = memo(function SyncedVerseRow({
  verse,
  isActive,
  mode,
  activeWordPosition,
  ...
  const stableWords = useMemo(() => verse.words.length > 0
    ? verse.words
    : verse.arabic.split(/\s+/).filter(Boolean).map((text, index) => ({ position: index + 1, text })), [verse]);
  const translationExcerpt = verse.translation ? clipWords(verse.translation, 10) : '';
  ...
  return (
    <Reanimated.View style={[styles.row, animatedStyle]}>
      <View style={styles.verseTop}>
        ...
        <Text style={styles.statusText}>{isActive ? 'En cours' : 'À suivre'}</Text>
      </View>
      {mode === 'arabic' ? (
        <>
          <StableArabicVerse
            ...
            preferredArabicSize={preferredArabicSize}
          />
          {translationExcerpt ? <Text numberOfLines={2} style={styles.translation}>{translationExcerpt}</Text> : null}
        </>
      ) : (
        <Text numberOfLines={3} style={styles.translationOnly}>
