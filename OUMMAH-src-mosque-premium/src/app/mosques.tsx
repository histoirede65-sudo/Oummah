...
export default function MosquesScreen() {
  ...
  const [selectedMosque, setSelectedMosque] = useState<NearbyMosque | null>(
    null,
  );
  ...
  const toggleMosqueFavorite = async (
    ...
    try {
      await setMosqueFavorite(mosque, nextFavorite);
    } catch {
      setFavoriteMosqueIds((current) => {
        const next = new Set(current);
  ...
  useFocusEffect(
    useCallback(() => {
      ...
      const loadMosquePreferences = async () => {
        const [storedMainMosque, favoriteMosques] = await Promise.all([
    ...
    <SafeAreaView edges={['top']} style={styles.safeArea}>
      ...
      >
        <View style={styles.heroCard}>
          ...
          <LinearGradient
            colors={['rgba(35,21,46,0.78)', 'rgba(16,11,27,0.94)']}
            start={{ x: 0, y: 0 }}
            end={{ x: 1, y: 1 }}
          ...
          >
            ...
            <View style={styles.privacyPill}>
              <Ionicons name="lock-closed" size={12} color="#B9AFC1" />
            ...
            {renderedMosques.map((mosque) => (
              ...
              >
                <View style={styles.mosqueImageWrap}>
                  <Image
                    source={getMosqueImage(mosque.id)}
                    resizeMode="cover"
...
const styles = StyleSheet.create({
  ...
  heroGlassPanel: {
    ...
    borderRadius: 24,
    borderWidth: 1,
    borderColor: 'rgba(240,209,140,0.36)',
    shadowColor: '#000',
    shadowOpacity: 0.42,
    shadowRadius: 20,
  ...
  heroLocationIcon: {
    ...
    borderRadius: 21,
    borderWidth: 1,
    borderColor: 'rgba(229,185,88,0.48)',
    backgroundColor: 'rgba(113,66,135,0.34)',
  },
  privacyPill: {
    marginTop: 10,
    flexDirection: 'row',
