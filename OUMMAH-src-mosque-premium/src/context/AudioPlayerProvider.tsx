...
async function withLocalDownload(track: AudioTrack): Promise<AudioTrack> {
  const download = await audioDependencies.repositories.downloads.get(track.id).catch(() => null);
...
function CompatibilityBridge({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    miniPlayerController.syncPlayback(track !== null, isPlaying);
  ...
  }, [currentTime, duration, isPlaying, playbackRate, player, repeatMode, tadabburMode, track]);
  ...
  const loadSurah = useCallback(async (surahId: number, autoplay = false, reciterId?: string) => {
    ...
    if (!playlist) {
      const track = await withLocalDownload(await audioDependencies.catalog.getTrack(surahId, resolvedReciterId));
    ...
    if (item) {
      ...
      const localizedPlaylist = localizedTrack === item.track
        ? playlist
        : {
          ...playlist,
          items: playlist.items.map((candidate) => (
            candidate.id === item.id ? { ...candidate, track: localizedTrack } : candidate
          )),
        };
      if (localizedPlaylist !== playlist) playlists.current.set(resolvedReciterId, localizedPlaylist);
  ...
  const nextSurah = useCallback(async () => {
    ...
    const reciterId = player.track ? getTrackReciter(player.track).id : currentReciter?.id;
  ...
  const value = useMemo<GlobalAudioPlayerContextValue>(() => {
    ...
    return { ...state, next: nextSurah, previous: previousSurah, loadSurah, miniPlayerState, hideMiniPlayer, showMiniPlayer, setFullPlayerActive, listeningResume, resumeListening, startNewListening };
  }, [hideMiniPlayer, listeningResume, loadSurah, miniPlayerState, nextSurah, player, previousSurah, resumeListening, setFullPlayerActive, showMiniPlayer, startNewListening]);
