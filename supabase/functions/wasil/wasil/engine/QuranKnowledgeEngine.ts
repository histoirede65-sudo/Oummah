...
async function searchQuranTerms(
  ...
  const collected = new Map<string, QuranTopicSource>();
  ...
  console.log("WASIL_QURAN_RELEVANCE_RANKING", {
    ...
    retained: ranked.map(({ item: [id, source], score, matchedTerms }) => ({
      id,
      reference: source.reference,
  ...
  return Object.fromEntries(ranked.map(({ item }) => item));
}
...
export async function retrieveQuranKnowledge(
  ...
  const queryTerms = expansion?.quranSearchTerms?.length
    ...
    : entity
    ? [...entity.searchTerms, ...entity.aliases.slice(0, 2)]
  ...
  if (queryTerms.length > 0) {
    try {
      dynamicSources = await searchQuranTerms(
        [...new Set(queryTerms)],
        expansion?.canonicalName ?? curated?.canonicalName ?? entity?.canonicalName ?? genericTerm,
      );
    } catch (error) {
  ...
  const finalRanked = rankDocuments(
    ...
    ([, source]) => ({
      canonicalName: expansion?.canonicalName ?? curated?.canonicalName ?? entity?.canonicalName ?? genericTerm,
