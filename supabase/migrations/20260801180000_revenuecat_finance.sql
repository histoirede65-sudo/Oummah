-- OUMMAH — RevenueCat réel, finances et alertes Wasil
-- Les revenus sont enregistrés à partir des événements webhook reçus.
...
create table if not exists public.revenuecat_customer_subscriptions (
...
create index if not exists revenuecat_subscriptions_active_idx
on public.revenuecat_customer_subscriptions(active, expiration_at);

alter table public.revenuecat_webhook_events enable row level security;
alter table public.revenuecat_customer_subscriptions enable row level security;

revoke all on public.revenuecat_webhook_events from anon, authenticated;
revoke all on public.revenuecat_customer_subscriptions from anon, authenticated;
...
revoke all on function
  public.admin_get_revenuecat_finance_dashboard()
from public;
